...
south = los.obc_segment('segment_001', momgrd,istart=0,iend=50,jstart=0,  jend=0  )
north = los.obc_segment('segment_002', momgrd,istart=50,iend=0,jstart=42,jend=42)
west  = los.obc_segment('segment_003', momgrd,istart=0,iend=0,  jstart=42,  jend=0)
...
#	list_vectvariables = [vel_south]
	...
	# ---------- write to file -----------------------------------
	ncdf.write_obc_file(list_segments,list_variables,list_vectvariables,time,output='obc_d' + mm + '_CCS0.nc')

# ---------- concat to a single file ---------------------------------
cmdcat = 'ncrcat obc_d??_CCS0.nc -O -o obc_monthly_CCS0.nc'
cmdmod = "ncatted -h -a modulo,time,c,c,' ' obc_monthly_CCS0.nc"
cmdclean = 'rm obc_d??_CCS0.nc'
