...
sgrid=supergrid(file='input_data/ocean_hgrid.nc')
grid_in=quadmesh(supergrid=sgrid,cyclic=True)
grid.D=nc.Dataset('ocean_topog.nc').variables['depth'][:]
S=state('input_data/global_3d_data.nc',grid=grid_in,fields=['thetao','so','uo','vo'],time_indices=np.arange(0,1))
SSH=state('input_data/global_ssh_data.nc',grid=grid_in,fields=['ssh'],time_indices=np.arange(0,1))
...
#plt.pcolormesh(grid.x_T,grid.y_T,sq(R.temp[0,0,:]));plt.colorbar()
#plt.show()
#raise
...
R.temp=np.ma.filled(R.temp,0.0)
R.salt=np.ma.filled(R.salt,0.0)
R.ssh=np.ma.filled(R.ssh,0.0)
